...
namespace Compress.Support.Compression.Explode
{
    public class ExplodeStream : Stream
    {
        ...
        private readonly int generalPurposeBitFlag;
        private readonly Stream inStream;

        private huft[] hufLiteralCodeTable; /* literal code table */
        private huft[] hufLengthCodeTable; /* length code table */
        private huft[] hufDistanceCodeTable; /* distance code table */
        ...
        public ExplodeStream(Stream inStr, ulong compressedSize, ulong uncompressedSize, ushort generalPurposeBitFlag)
        {
            inStream = inStr;
        ...
        private int explode_SetTables()
        {
            ...
            /* With literal tree--minimum match length is 3 */
            {
                ...
                if ((returnCode = HuftTree.huftbuid(arrBitLengthsForCodes, 256, 256, null, null,
                        out hufLiteralCodeTable, ref bitsForLiteralCodeTable)) != 0)
                    return returnCode;

                if ((returnCode = get_tree(arrBitLengthsForCodes, 64)) != 0)
                    return returnCode;

                if ((returnCode = HuftTree.huftbuid(arrBitLengthsForCodes, 64, 0, cplen3, extra,
                        out hufLengthCodeTable, ref bitsForLengthCodeTable)) != 0)
                    return returnCode;
            }
            else
            /* No literal tree--minimum match length is 2 */
            {
                ...
                if ((returnCode = HuftTree.huftbuid(arrBitLengthsForCodes, 64, 0, cplen2, extra,
                        out hufLengthCodeTable, ref bitsForLengthCodeTable)) != 0)
                    return returnCode;
            }
            ...
            if ((generalPurposeBitFlag & 2) != 0) /* true if 8K */
            {
                numOfUncodedLowerDistanceBits = 7;
                returnCode = HuftTree.huftbuid(arrBitLengthsForCodes, 64, 0, cpdist8, extra,
                    out hufDistanceCodeTable, ref bitsForDistanceCodeTable);
            }
            else /* else 4K */
            {
                numOfUncodedLowerDistanceBits = 6;
                returnCode = HuftTree.huftbuid(arrBitLengthsForCodes, 64, 0, cpdist4, extra,
                    out hufDistanceCodeTable, ref bitsForDistanceCodeTable);
            }
        ...
        int DecodeHuft(huft[] htab, int bits, uint mask, out huft huftPointer, out int e)
        {
            NeedBits(bits);
            ...
            while (countIndex < count && outBytesCount < unCompressedSize) /* do until unCompressedSize bytes uncompressed */
            {
                if (length == 0)
                {
                    ...
                    if (literal) /* then literal--decode it */
                    {
                        ...
                        {
                            /* get coded literal */
                            if (DecodeHuft(hufLiteralCodeTable, bitsForLiteralCodeTable, maskForLiteralCodeTable, out huftPointer, out _) != 0)
                    ...
                    if (DecodeHuft(hufDistanceCodeTable, bitsForDistanceCodeTable, maskForDistanceCodeTable, out huftPointer, out _) != 0)
                        throw new Exception("Error decoding distance high bits");

                    distance = windowIndex - (distance + huftPointer.Value); /* construct offset */

                    /* get coded length */
                    if (DecodeHuft(hufLengthCodeTable, bitsForLengthCodeTable, maskForLengthCodeTable, out huftPointer, out int extraBitLength) != 0)

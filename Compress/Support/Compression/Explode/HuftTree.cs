...
namespace Compress.Support.Compression.Explode
{
    public class huft
    {
        ...
        public huft[] ChildNodes; /* next level of table */
    }
    ...
    public static class HuftTree
    {
        ...
        public static int huftbuid(int[] arrBitLengthForCodes, int numberOfCodes, int numberOfSimpleValueCodes, int[] arrBaseValuesForNonSimpleCodes, int[] arrExtraBitsForNonSimpleCodes,
        out huft[] outHufTable, ref int outBitsForTable)
        ...
        decoded. */
        {
            ...
            do
            {
                arrBitLengthCount[arrBitLengthForCodes[pIndex]]++; pIndex++; /* assume all entries <= BMAX */
            ...
            for (numberOfDummyCodesAdded = 1 << counter; counter < counterCurrentCode; counter++, numberOfDummyCodesAdded <<= 1)
            ...
            huft[][] arrHufTableStack = new huft[BMAX][];
            arrHufTableStack[0] = null; /* just to keep compilers happy */
            huft[] pointerToCurrentTable = null; /* ditto */
            int numberOfEntriesInCurrentTable = 0; /* ditto */
            ...
            for (; numberOfBitsInCurrentCode <= maximumCodeLength; numberOfBitsInCurrentCode++)
            {
                ...
                while ((counterForCodes--) != 0)
                {
                    ...
                    while (numberOfBitsInCurrentCode > bitsBeforeThisTable + arrLX[stackOfBitsPerTable + tableLevel])
                    {
                        ...
                        numberOfEntriesInCurrentTable = (numberOfEntriesInCurrentTable = maximumCodeLength - bitsBeforeThisTable) > outBitsForTable ? outBitsForTable : numberOfEntriesInCurrentTable; /* upper limit */
                        ...
                        if (bitsBeforeThisTable + counter > lengthOfEOBcode && bitsBeforeThisTable < lengthOfEOBcode)
                        ...
                        pointerToCurrentTable = new huft[numberOfEntriesInCurrentTable];
                        ...
                        if (tableLevel != 0)
                        {
                            ...
                            counter = (counterCurrentCode & ((1 << bitsBeforeThisTable) - 1)) >> (bitsBeforeThisTable - arrLX[stackOfBitsPerTable + tableLevel - 1]);
                            arrHufTableStack[tableLevel - 1][counter] = vHuft; /* connect to last table */
                        }
                    }

                    /* set up table entry in r */
                    huft vHuft1 = new huft
                    {
                    ...
                    {
                        vHuft1.NumberOfExtraBits = arrExtraBitsForNonSimpleCodes[arrValuesInOrderOfBitLength[pIndex] - numberOfSimpleValueCodes]; /* non-simple--look up in lists */
                        vHuft1.Value = arrBaseValuesForNonSimpleCodes[arrValuesInOrderOfBitLength[pIndex++] - numberOfSimpleValueCodes];
                    }

                    /* fill code-like entries with r */
                    int fBitCounter2 = 1 << (numberOfBitsInCurrentCode - bitsBeforeThisTable);
                    for (counter = counterCurrentCode >> bitsBeforeThisTable; counter < numberOfEntriesInCurrentTable; counter += fBitCounter2)
                        pointerToCurrentTable[counter] = vHuft1;

                    /* backwards increment the k-bit code i */
                    for (counter = 1 << (numberOfBitsInCurrentCode - 1); (counterCurrentCode & counter) != 0; counter >>= 1)

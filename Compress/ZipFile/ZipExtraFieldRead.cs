...
{
    internal static class ZipExtraFieldRead
    {
        public static ZipReturn ExtraFieldRead(ZipFileData lf, bool centralDir)
        {
            int extraFieldLength = lf.bExtraField.Length;
            ...
            {
                ushort type = BitConverter.ToUInt16(lf.bExtraField, blockPos);
                blockPos += 2;
                ushort blockLength = BitConverter.ToUInt16(lf.bExtraField, blockPos);
                blockPos += 2;
                ...
                switch (type)
                {
                    ...
                    */
                    case 0x0001:
                        ...
                        {
                            lf.UncompressedSize = BitConverter.ToUInt64(lf.bExtraField, pos);
                            pos += 8;
                        }
                        if (lf.CompressedSize == 0xffffffff)
                        {
                            lf.CompressedSize = BitConverter.ToUInt64(lf.bExtraField, pos);
                            pos += 8;
                        }
                        if (centralDir)
                        {
                            ...
                            {
                                lf.RelativeOffsetOfLocalHeader = BitConverter.ToUInt64(lf.bExtraField, pos);
                                pos += 8;
                            }
                        }
                        break;
                    ...
                    */
                    case 0x000a:
                        ...
                        int tag1 = BitConverter.ToInt16(lf.bExtraField, pos); // Tag1          Short       NTFS attribute tag value #1
                        pos += 2;
                        int size1 = BitConverter.ToInt16(lf.bExtraField, pos); // Size1         Short       Size of attribute #1, in bytes
                        pos += 2;
                        if (tag1 == 0x0001 && size1 == 24
                        ) // (currently only one set of attributes is defined for NTFS)
                        {
                            lf.ModifiedTime = CompressUtils.UtcTicksFromNtfsDateTime(BitConverter.ToInt64(lf.bExtraField, pos)); // Mtime      8 bytes    64-bit NTFS file last modification time
                            pos += 8;
                            lf.AccessedTime = CompressUtils.UtcTicksFromNtfsDateTime(BitConverter.ToInt64(lf.bExtraField, pos)); // Atime      8 bytes    64-bit NTFS file last access time
                            pos += 8;
                            lf.CreatedTime = CompressUtils.UtcTicksFromNtfsDateTime(BitConverter.ToInt64(lf.bExtraField, pos)); // Ctime      8 bytes    64-bit NTFS file creation time
                            pos += 8;
                    ...
                    */
                    case 0x5455:
                        ...
                        byte flags = lf.bExtraField[pos];
                        pos += 1;
                        if ((flags & 1) == 1)
                        {
                            lf.ModifiedTime = CompressUtils.UtcTicksFromUnixDateTime(BitConverter.ToInt32(lf.bExtraField, pos)); // (ModTime)     Long        time of last modification (UTC/GMT)
                        ...
                        if (!centralDir)
                        {
                            if ((flags & 2) == 2)
                            {
                                lf.AccessedTime = CompressUtils.UtcTicksFromUnixDateTime(BitConverter.ToInt32(lf.bExtraField, pos)); // (AcTime)      Long        time of last access (UTC/GMT)
                            ...
                            if ((flags & 4) == 4)
                            {
                                lf.CreatedTime = CompressUtils.UtcTicksFromUnixDateTime(BitConverter.ToInt32(lf.bExtraField, pos)); // (CrTime)      Long        time of original creation (UTC/GMT)
                    ...
                    */
                    case 0x7075:
                        ...
                        byte version = lf.bExtraField[pos];
                        pos += 1;
                        uint nameCRC32 = BitConverter.ToUInt32(lf.bExtraField, pos);
                        pos += 4;

                        CRC crcTest = new();
                        crcTest.SlurpBlock(lf.bFileName, 0, lf.bFileName.Length);
                        uint fCRC = crcTest.Crc32ResultU;

                        if (nameCRC32 == fCRC)
                        {
                            int charLen = blockLength - 5;
                            lf.Filename = Encoding.UTF8.GetString(lf.bExtraField, pos, charLen);
                        }

...
namespace RVCore.FindFix
{
    public class FileGroup
    {
        ...
        private bool FindAltExactMatch(RvFile file)
        {
            ...
            if (!Equal(file.AltSize, Size)) return false;
            if (!ArrByte.ECompare(file.AltCRC, CRC)) return false;
            if (!ArrByte.ECompare(file.AltSHA1, SHA1)) return false;
            if (!ArrByte.ECompare(file.AltMD5, MD5)) return false;

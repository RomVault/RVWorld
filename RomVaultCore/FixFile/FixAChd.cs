...
public static class FixAChd
{
    ...
    private static bool TryBuildChdFromAudioTracks(RvFile chdDir, List<RvFile> fileProcessQueue, ref int totalFixed, out string errorMessage)
    {
        ...
        string encodedRoot = GetEncodedMediaRootName(chdDir.Name);
        string encodedExtension = System.IO.Path.GetExtension(encodedRoot ?? "").ToLowerInvariant();
        if (encodedExtension == ".iso" || encodedExtension == ".gdi")
        {
    ...
    private static RvFile FindSiblingDiscSource(RvFile chdDir)
    {
        ...
        if (setDir == null || !setDir.IsDirectory)
            return null;
        string encodedRoot = GetEncodedMediaRootName(chdDir.Name);
        if (!string.IsNullOrWhiteSpace(encodedRoot) && setDir.ChildNameSearch(FileType.File, encodedRoot, out int idxRoot) == 0)
        {
            RvFile root = setDir.Child(idxRoot);
            if (root != null && root.IsFile && root.GotStatus == GotStatus.Got)
                return root;
        }
    ...
    private static List<RvFile> GetToSortSearchRoots(RvFile chdDir)
    {
        ...
        string setName = chdDir.Parent?.Name ?? "";
        if (string.IsNullOrWhiteSpace(setName))
        {
            setName = chdDir.Name ?? "";
            if (setName.EndsWith(".chd", StringComparison.OrdinalIgnoreCase))
        ...
        string categoryName = chdDir.Parent?.Parent?.Name ?? "";
    ...
    private static RvFile FindByHash(HashIndex idx, RvFile expected)
    {
        ...
        {
            string key = expected.SHA1.ToHexString();
        ...
        {
            string key = expected.MD5.ToHexString();
        ...
        {
            string key = expected.CRC.ToHexString();
    ...
    private static bool IsStrongHashMatch(RvFile expected, RvFile got)
    {
        ...
        if (expected.SHA1 != null && expected.SHA1.Length > 0)
        {
            if (got.FileStatusIs(FileStatus.SHA1Verified) && ByteUtils.ByteArrEquals(expected.SHA1, got.SHA1))
                return true;
            if (got.FileStatusIs(FileStatus.AltSHA1Verified) && ByteUtils.ByteArrEquals(expected.SHA1, got.AltSHA1))
                return true;
            return false;
        }
        if (expected.MD5 != null && expected.MD5.Length > 0)
        {
            if (got.FileStatusIs(FileStatus.MD5Verified) && ByteUtils.ByteArrEquals(expected.MD5, got.MD5))
                return true;
            if (got.FileStatusIs(FileStatus.AltMD5Verified) && ByteUtils.ByteArrEquals(expected.MD5, got.AltMD5))
                return true;
            return false;
        }
        if (expected.CRC != null && expected.CRC.Length > 0)
        {
            if (got.FileStatusIs(FileStatus.CRCVerified) && ByteUtils.ByteArrEquals(expected.CRC, got.CRC))
                return true;
            if (got.FileStatusIs(FileStatus.AltCRCVerified) && ByteUtils.ByteArrEquals(expected.CRC, got.AltCRC))
                return true;
            return false;
        }
        return false;
    ...
    private static RvFile FindDiscSourceInDirs(RvFile chdDir, List<RvFile> dirs)
    {
        ...
        {
            RvFile exactChd = FindFileByNameInDirs(dirs, chdDir.Name);
            if (exactChd != null && exactChd.GotStatus == GotStatus.Got)
                return exactChd;
            RvFile exactRoot = FindFileByNameInDirs(dirs, encodedRoot);
            if (exactRoot != null && exactRoot.GotStatus == GotStatus.Got)
                return exactRoot;
            return null;
        }
    ...
    private static string GetEncodedMediaRootName(string chdName)
    {
        if (string.IsNullOrWhiteSpace(chdName) || !chdName.EndsWith(".chd", StringComparison.OrdinalIgnoreCase))
            return null;
        string rootName = System.IO.Path.GetFileNameWithoutExtension(System.IO.Path.GetFileName(chdName));
        string extension = System.IO.Path.GetExtension(rootName).ToLowerInvariant();
        switch (extension)
        {
            case ".cue":
            case ".gdi":
    ...
    private static bool MatchesEncodedMediaRoot(RvFile source, string expectedChdName, string encodedRoot)
    {
        if (source == null || string.IsNullOrWhiteSpace(encodedRoot))
            return source != null;
        if (source.FileType == FileType.CHD)
            return string.Equals(source.Name, expectedChdName, StringComparison.OrdinalIgnoreCase);
        return string.Equals(System.IO.Path.GetFileName(source.Name ?? ""), encodedRoot, StringComparison.OrdinalIgnoreCase);
    }

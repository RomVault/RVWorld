...
namespace RomVaultCore.FixFile.Utils
{
    public static class Decompress7ZipFile
    {
        ...
        public static ReturnCode DecompressSource7ZipFile(RvFile db7zFile, bool includeGood, Dictionary<string, RvFile> filesUsedForFix, out string error)
        {
            ...
            for (int i = 0; i < sevenZipFileCaching.LocalFilesCount; i++)
            {
                ...
                int errorCode = FileStream.OpenFileWrite(filenameOut, FileStream.BufSizeMax, out Stream writeStream);
                ...
                while (sizetogo > 0)
                {
                    ...
                    {
                        error = "Decompress7Zip File: Error writing out file. " + Environment.NewLine + e.Message;
                        return ReturnCode.FileSystemError;
                    }
                    tcrc32.Wait();
                    tmd5?.Wait();

                        ...
                        for (int indexCase = (caseTest ? 0 : 1); indexCase < 2; indexCase += 1)
                        {
                            for (int indexfile = 0; indexfile < filesCount; indexfile++)
                            {
                                ...
                                for (int indexdb = 0; indexdb < dbsCount; indexdb++)
                                {
                                    ...
                                    bool matched = Scanner.Compare.Phase2Test(dbs[indexdb], files[indexfile], EScanLevel, indexCase, dbDir.FullNameCase, _thWrk, ref _fileErrorAbort, out bool matchedAlt);
                                    if (!matched)
                                    {
                                        continue;
                                    }

...
namespace RVXCore.DB
{
    public class RvDir
    {
        ...
        private static uint? FindInDir(string fullname)
        {
            ...
            {
                CommandFindInDir = new SQLiteCommand(@"SELECT DirId FROM dir WHERE fullname=@fullname LIMIT 1",  DBSqlite.db.Connection);
        ...
        private static void SetDirFound(uint foundDatId)
        {
            ...
            {
                CommandSetDirFound = new SQLiteCommand(@"Update Dir SET Found=1 WHERE DirId=@DirId",  DBSqlite.db.Connection);
